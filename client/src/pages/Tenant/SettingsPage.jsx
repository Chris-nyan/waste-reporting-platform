...
const ProfileTab = () => {
    ...
    const onProfileSubmit = async (data) => {
        try {
            ...
            toast.success(t('settings.profile.toast_success', "Profile updated successfully!"));
        } catch (error) {
            toast.error(t('settings.profile.toast_error', "Failed to update profile."));
        }
    };
    ...
    const onPasswordSubmit = async (data) => {
        try {
            ...
            toast.success(t('settings.security.toast_success', "Password changed successfully!"));
            passwordForm.reset();
        } catch (error) {
            toast.error(error.response?.data?.message || t('settings.security.toast_error', "Failed to change password."));
        }
    };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <Form {...profileForm}>
                        <form onSubmit={profileForm.handleSubmit(onProfileSubmit)} className="space-y-4">
                            <FormField control={profileForm.control} name="email" render={({ field }) => (<FormItem>
                                <FormLabel>{t('settings.profile.email', 'Email Address')}</FormLabel>
            ...
            <Card className="bg-white/70 backdrop-blur-xl border border-gray-200/50 shadow-sm rounded-xl">
                <CardHeader>
                    ...
                    <CardDescription>{t('settings.security.description', 'Update your account password.')}</CardDescription>
                </CardHeader>
                <CardContent>
                    <Form {...passwordForm}>
                        <form onSubmit={passwordForm.handleSubmit(onPasswordSubmit)} className="space-y-4">
                            <FormField control={passwordForm.control} name="currentPassword" render={({ field }) => (<FormItem>
                                <FormLabel>{t('settings.security.current', 'Current Password')}</FormLabel>
                            ...
                            <FormField control={passwordForm.control} name="newPassword" render={({ field }) => (<FormItem>
                                <FormLabel>{t('settings.security.new', 'New Password')}</FormLabel>
                                <FormControl><Input type="password" {...field} /></FormControl><FormMessage /></FormItem>)} />
                            <FormField control={passwordForm.control} name="confirmPassword" render={({ field }) => (<FormItem>
                                <FormLabel>{t('settings.security.confirm', 'Confirm New Password')}</FormLabel>
                                <FormControl><Input type="password" {...field} /></FormControl><FormMessage /></FormItem>)} />
...
const CrudSection = ({ title, endpoint, fields, icon }) => {
    ...
    const fetchData = useCallback(async () => {
        try { setLoading(true); const response = await api.get(endpoint); setItems(response.data); } catch (error) {
            (t('settings.crud.error_load', { title: title.toLowerCase() }));
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            toast.success(t('settings.crud.delete_success', { title: title.slice(0, -1) }));
            setItemToDelete(null); fetchData();
        } catch (error) {
            toast.error(t('settings.crud.delete_error', 'Failed to delete item.'));
        }
    };
        ...
        <Card className="bg-white/70 backdrop-blur-xl border border-gray-200/50 shadow-sm rounded-xl h-full">
            <CardHeader className="flex flex-row items-center justify-between">
                <div className="space-y-1.5">
                    ...
                    <CardDescription>{t('settings.crud.description', { title: title.toLowerCase() })}</CardDescription>
                </div>
                ...
                {loading ? <div className="flex justify-center p-8"><Loader2 className="h-6 w-6 animate-spin text-emerald-500" /></div> : (
                    <Table>
                        <TableHeader><TableRow>
                            <TableHead>{t('settings.crud.col_name', 'Name')}</TableHead>
                            {fields.length > 1 && <TableHead>{t('settings.crud.col_address', 'Address')}</TableHead>}
                            <TableHead className="text-right w-[100px]">{t('settings.crud.col_actions', 'Actions')}</TableHead>
                        </TableRow>
                        </TableHeader>
                        <TableBody>
            ...
            <ConfirmationDialog open={!!itemToDelete} onOpenChange={() => setItemToDelete(null)} onConfirm={handleDelete}
                title={t('settings.crud.confirm_delete_title', { title: title.slice(0, -1) })}
                description={t('settings.crud.confirm_delete_desc', 'This action cannot be undone. Are you sure you want to permanently delete this item?')}
...
const AddItemDialog = ({ fields, endpoint, title, onAdded }) => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            toast.success(t('settings.crud.add_success', { title: title.slice(0, -1) }));

            onAdded(); form.reset(); setIsOpen(false);
        } catch (error) {
            toast.error(t('settings.crud.add_error', 'Failed to add item.'));
        }
    };
    return (<Dialog open={isOpen} onOpenChange={setIsOpen}><DialogTrigger asChild>
        <Button className="bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-sm hover:shadow-lg"><PlusCircle className="mr-2 h-4 w-4" /> {t('settings.crud.add_new', 'Add New')}</Button>
    </DialogTrigger><DialogContent><DialogHeader>
        <DialogTitle>{t('settings.crud.add_title', { title: title.slice(0, -1) })}</DialogTitle>
    </DialogHeader><Form {...form}><form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">{fields.map(field => (<FormField key={field.name} control={form.control} name={field.name} render={({ field: formField }) => (<FormItem><FormLabel>{t(field.label, field.label)}</FormLabel><FormControl><Input placeholder={t(field.placeholder, field.placeholder)} {...formField} /></FormControl><FormMessage /></FormItem>)} />))}<DialogFooter><DialogClose asChild>
        <Button type="button" variant="outline">{t('settings.crud.cancel', 'Cancel')}</Button></DialogClose>
        <Button className="bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-md hover:shadow-lg transform hover:scale-[1.02] transition-all duration-200"
            type="submit" disabled={form.formState.isSubmitting}>{form.formState.isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{t('settings.crud.save', 'Save')}</Button>
...
const EditItemDialog = ({ item, isOpen, onOpenChange, onUpdated, fields, title, endpoint }) => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            toast.success(t('settings.crud.update_success', { title: title.slice(0, -1) }));
            onUpdated(); onOpenChange(false);
        } catch (error) {
            toast.error(t('settings.crud.update_error', 'Failed to update item.'));
        }
    };
    return (<Dialog open={isOpen} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>{t('settings.crud.edit_title', { title: title.slice(0, -1) })}</DialogTitle></DialogHeader><Form {...form}><form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">{fields.map(field => (<FormField key={field.name} control={form.control} name={field.name} render={({ field: formField }) => (<FormItem><FormLabel>{t(field.label, field.label)}</FormLabel><FormControl><Input placeholder={t(field.placeholder, field.placeholder)} {...formField} /></FormControl><FormMessage /></FormItem>)} />))}<DialogFooter><DialogClose asChild><Button type="button" variant="outline">{t('settings.crud.cancel', 'Cancel')}</Button></DialogClose><Button className="bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-md hover:shadow-lg transform hover:scale-[1.02] transition-all duration-200"
        type="submit" disabled={form.formState.isSubmitting}>{form.formState.isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
        {t('settings.crud.save_changes', 'Save Changes')}
...
const SettingsPage = () => {
    ...
    const facilityFields = [
        { name: 'name', label: 'settings.locations.facility_name', placeholder: 'settings.locations.facility_placeholder' },
        { name: 'fullAddress', label: 'settings.locations.address_label', placeholder: 'settings.locations.address_placeholder' }
    ];
    const pickupFields = [
        { name: 'name', label: 'settings.locations.pickup_name', placeholder: 'settings.locations.pickup_placeholder' },
        { name: 'fullAddress', label: 'settings.locations.address_label', placeholder: 'settings.locations.address_placeholder' }
    ];
    const vehicleFields = [
        { name: 'name', label: 'settings.vehicles.name_label', placeholder: 'settings.vehicles.name_placeholder' }
    ];
    return (
